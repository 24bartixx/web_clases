...
export default {
  ...
  data() {
    return {
      form: {
        ...
        surname: this.author? this.author.surname : "",
      },
      errors: {},
      error: null,
      authorConstraints: {
  ...
  methods: {
    async fetchValidationRules() {
      try {
        const res = await fetch(`${API_URL}/validationRules/author`);
        if (!res.ok) throw new Error("Failed to fetch rules");
        this.authorConstraints = await res.json();
      } catch (e) {
        this.error = "Failed to load validation rules.";
      }
    },
    validateForm() {
      const errors = {};
      const { name, surname } = this.form;
      const c = this.authorConstraints;
      
      if (!name || name.length < c.minNameLength || name.length > c.maxNameLength) {
        errors.name = `Name must be between ${c.minNameLength} and ${c.maxNameLength} characters`;
      }
      if (!surname || surname.length < c.minSurnameLength || surname.length > c.maxSurnameLength) {
    ...
    async onSubmit() {
      ...
      try {
        const res = await fetch(`${API_URL}/authors/${this.author.id}`, {
          method: "PUT",
